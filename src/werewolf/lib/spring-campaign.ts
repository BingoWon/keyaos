import { SPRING_CAMPAIGN_ENABLED } from "@wolf/lib/welfare-config";
...
export function getSpringQuotaExpiresAtIso(quotaDate: string) {
	...
	const nextDayMidnightUtc = new Date(
		Date.UTC(year, month - 1, day + 1, -8, 0, 0),
	);
...
export function buildSpringCampaignBase(
	now: Date = new Date(),
): SpringCampaignSnapshot {
