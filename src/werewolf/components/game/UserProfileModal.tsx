...
import {
	...
	DropdownMenuContent,
	DropdownMenuTrigger,
} from "@wolf/components/ui/dropdown-menu";
...
import { getModelLogoPath } from "@wolf/lib/model-logo";
...
export function UserProfileModal({
	...
	onCreditsChange: _onCreditsChange,
	defaultTab = "profile",
	...
	const [_isWechatQrOpen, _setIsWechatQrOpen] = useState(false);
	const [redeemCodeInput, setRedeemCodeInput] = useState("");
	const [isRedeeming, setIsRedeeming] = useState(false);
	const profileActionGridClassName = REFERRAL_BONUS_ENABLED
		? "grid grid-cols-2 gap-2"
		: "grid grid-cols-1 gap-2";
	const shouldShowSpringCampaignQuota =
		SPRING_CAMPAIGN_ENABLED && springCampaign?.active;
	const shouldShowSpringCampaignStatus = SPRING_CAMPAIGN_ENABLED;

	const displayCredits = useMemo(() => {
		if (credits === null || credits === undefined)
			return t("userProfile.empty");
		return `${credits}`;
	}, [credits, t]);
	...
	useEffect(() => {
		...
		setSelectedModelsState((prev) => {
			...
			return defaultPlayerModels
				.map((ref) => ref.model)
				.filter((m) => playerSet.has(m));
		});
	...
	const handleSaveKeys = () => {
		if (isCustomKeyEnabled) {
			const zenmuxOk =
				!zenmuxKey.trim() || validatedKeys.zenmux === zenmuxKey.trim();
			const dashscopeOk =
				!dashscopeKey.trim() || validatedKeys.dashscope === dashscopeKey.trim();
			const newapiOk =
				!newapiKey.trim() || validatedKeys.newapi === newapiKey.trim();
			if (!zenmuxOk || !dashscopeOk || !newapiOk) {
				toast(t("customKey.toasts.notValidated"), {
					description: t("customKey.toasts.notValidatedDesc"),
				});
				return;
			}
		}
		...
		if (isCustomKeyEnabled && availableSet.size === 0) {
			...
			toast(t("customKey.toasts.needLlmKey"), {
				description: t("customKey.toasts.needLlmKeyDesc"),
			});
			return;
		}
		const nextSelectedModels = selectedModels.filter((m) =>
			playerAvailableSet.has(m),
		);
		...
		const fallbackReview = availableSet.has(REVIEW_MODEL)
			? REVIEW_MODEL
			: (availableModelPool[0]?.model ?? "");
		const nextGeneratorModel = availableSet.has(generatorModel)
			? generatorModel
			: fallbackGenerator;
		const nextSummaryModel = availableSet.has(summaryModel)
			? summaryModel
			: fallbackSummary;
		const nextReviewModel = availableSet.has(reviewModel)
			? reviewModel
			: fallbackReview;
		const removedSelected = selectedModels.filter(
			(m) => !playerAvailableSet.has(m),
		);
		const generatorAdjusted =
			Boolean(generatorModel) && !availableSet.has(generatorModel);
		const summaryAdjusted =
			Boolean(summaryModel) && !availableSet.has(summaryModel);
		const reviewAdjusted =
			Boolean(reviewModel) && !availableSet.has(reviewModel);
		...
		setSummaryModelState(nextSummaryModel);
		setReviewModelState(nextReviewModel);
		toast(t("customKey.toasts.saved"), {
	...
	const handleValidateZenmux = async () => {
		...
		try {
			...
			if (zenmuxKey.trim() === getValidatedZenmuxKey())
				setValidatedZenmuxKey("");
	...
	const handleValidateDashscope = async () => {
		...
		try {
			...
			if (dashscopeKey.trim() === getValidatedDashscopeKey())
				setValidatedDashscopeKey("");
			toast(t("customKey.toasts.validateFailed"), {
				description: t("customKey.toasts.validateFailedDesc"),
			});
	...
	const handleValidateNewapi = async () => {
		if (isValidatingNewapi || !newapiKey.trim() || !newapiBaseUrl.trim())
			return;
		...
		try {
			...
			if (newapiKey.trim() === getValidatedNewapiKey())
				setValidatedNewapiKey("");
			toast(t("customKey.toasts.validateFailed"), {
				description: t("customKey.toasts.validateFailedDesc"),
			});
	...
	const handleRedeem = async () => {
		...
		try {
			...
			if (result.success) {
				toast(
					t("customKey.payAsYouGo.redeemSuccess", {
						count: result.creditsGranted ?? 5,
					}),
				);
				setRedeemCodeInput("");
			} else {
				const errorKey = result.error as
					...
					| undefined;
				const errorMsg =
					errorKey && t.has(`customKey.payAsYouGo.redeemError.${errorKey}`)
				...
				<Tabs defaultValue={defaultTab} key={defaultTab}>
					<TabsList>
						<TabsTrigger value="profile">
							{t("customKey.tabs.profile")}
						</TabsTrigger>
						<TabsTrigger value="payAsYouGo">
							{t("customKey.tabs.payAsYouGo")}
						</TabsTrigger>
						<TabsTrigger value="custom">
							{t("customKey.tabs.custom")}
						</TabsTrigger>
					</TabsList>
							...
							<div className="rounded-lg border-2 border-[var(--border-color)] bg-[var(--bg-card)] p-3 space-y-2 text-sm">
								<div className="flex items-center justify-between">
									...
									<span className="text-[var(--text-primary)]">
										{email ?? t("userProfile.loggedIn")}
									</span>
								</div>
								<div className="flex items-center justify-between">
									<span className="text-[var(--text-muted)]">
										{t("userProfile.fields.credits")}
									</span>
									<span className="text-[var(--text-primary)]">
										{displayCredits}
									</span>
								...
								{shouldShowSpringCampaignQuota && (
									<div className="flex items-center justify-between">
										<span className="text-[var(--text-muted)]">
											{t("userProfile.fields.springQuota")}
										</span>
										<span className="text-[var(--text-primary)]">
								...
								{REFERRAL_BONUS_ENABLED && (
									<>
										<div className="flex items-center justify-between">
											<span className="text-[var(--text-muted)]">
												{t("userProfile.fields.referrals")}
											</span>
											<span className="text-[var(--text-primary)]">
												{totalReferrals ?? 0}
											</span>
										...
										<div className="flex items-center justify-between gap-3">
											<span className="text-[var(--text-muted)]">
												{t("userProfile.fields.referralCode")}
											</span>
							...
							{REDEMPTION_CODE_ENABLED && (
								<>
									<section className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-4 space-y-4">
										<div>
											<h3 className="text-sm font-medium text-[var(--text-primary)]">
												{t("customKey.payAsYouGo.purchaseTitle")}
											</h3>
											<p className="text-xs text-[var(--text-muted)] mt-1">
												{t("customKey.payAsYouGo.purchaseDesc")}
											</p>
										</div>
									...
									<section className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-4 space-y-3">
										<div>
											<h3 className="text-sm font-medium text-[var(--text-primary)]">
												{t("customKey.payAsYouGo.redeemTitle")}
											</h3>
											<p className="text-xs text-[var(--text-muted)] mt-1">
												{t("customKey.payAsYouGo.redeemHint")}
											</p>
										</div>
										<div className="flex gap-2">
											<Input
												...
												placeholder={t(
													"customKey.payAsYouGo.redeemPlaceholder",
												)}
												className="flex-1"
											...
											>
												<CreditCard size={16} />
												{isRedeeming
													? t("customKey.payAsYouGo.redeeming")
													: t("customKey.payAsYouGo.redeemButton")}
											</Button>
										</div>
									</section>
								</>
							...
							<section className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-4 space-y-4">
								<div className="flex items-center justify-between">
									<span className="text-sm text-[var(--text-muted)]">
										{t("customKey.payAsYouGo.pricePerGame", { price: "0.50" })}
									</span>
								</div>

								<div className="space-y-2">
									<Label className="text-xs">
										{t("customKey.payAsYouGo.quantity")}
									</Label>
									<div className="flex items-center gap-2">
										<Button
											type="button"
									...
									<div className="space-y-1">
										<p className="text-xs text-[var(--text-muted)]">
											{t("customKey.payAsYouGo.minQuantity")}
										</p>
										<p className="text-xs text-[var(--text-muted)] opacity-70">
											{t("customKey.payAsYouGo.minQuantityHint")}
										</p>
									</div>
								</div>
								...
								<div className="border-t border-[var(--border-color)] pt-4">
									...
									>
										<CreditCard size={16} />
										{isPurchasing
											? t("customKey.payAsYouGo.redirecting")
											: t("customKey.payAsYouGo.purchase")}
									</Button>
								</div>
							</section>
						</div>
							...
							{isCustomKeyEnabled && (
								<>
									...
									<section className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-4 space-y-4">
										<div>
											<h3 className="text-sm font-medium text-[var(--text-primary)]">
												{t("customKey.llmKey.title")}
											</h3>
											<p className="text-xs text-[var(--text-muted)] mt-1">
												{t("customKey.llmKey.description")}
											</p>
										</div>

										<div className="space-y-2">
											<Label htmlFor="zenmux-key" className="text-xs">
												{t("customKey.zenmux.label")}
											</Label>
											...
											<div className="flex gap-2">
												...
												<Button
													...
													aria-label={
														showZenmuxKey
															? t("customKey.zenmux.hide")
															: t("customKey.zenmux.show")
													}
												>
													{showZenmuxKey ? (
														<EyeSlash size={16} />
												...
												<Button
													...
													variant="outline"
													size="sm"
													onClick={handleValidateZenmux}
													disabled={
														isValidatingZenmux ||
														!zenmuxKey.trim() ||
														(!!validatedKeys.zenmux &&
															validatedKeys.zenmux === zenmuxKey.trim())
													}
												>
													{isValidatingZenmux ? (
														t("customKey.validating")
													) : validatedKeys.zenmux &&
														validatedKeys.zenmux === zenmuxKey.trim() ? (
														<Check
															size={16}
										...
										<div className="border-t border-[var(--border-color)] pt-3 space-y-2">
											<Label htmlFor="dashscope-key" className="text-xs">
												{t("customKey.dashscope.label")}
											</Label>
											...
											<div className="flex gap-2">
												...
												<Button
													...
													aria-label={
														showDashscopeKey
															? t("customKey.dashscope.hide")
															: t("customKey.dashscope.show")
													}
												>
													{showDashscopeKey ? (
														<EyeSlash size={16} />
												...
												<Button
													...
													variant="outline"
													size="sm"
													onClick={handleValidateDashscope}
													disabled={
														isValidatingDashscope ||
														!dashscopeKey.trim() ||
														(!!validatedKeys.dashscope &&
															validatedKeys.dashscope === dashscopeKey.trim())
													}
												>
													{isValidatingDashscope ? (
														t("customKey.validating")
											...
											>
												...
												<div className="min-w-0 flex-1">
													<span className="text-xs font-medium text-[var(--text-primary)]">
														{t("customKey.dashscope.get")}
													</span>
													<span className="text-[11px] text-[var(--text-muted)] ml-1.5">
														{t("customKey.dashscope.note")}
													</span>
												</div>
												<ArrowRight
													size={14}
										...
										<div className="border-t border-[var(--border-color)] pt-3 space-y-2">
											<Label htmlFor="newapi-base-url" className="text-xs">
												New API Base URL
											</Label>
											<Input
												id="newapi-base-url"
											...
											<Label htmlFor="newapi-key" className="text-xs">
												New API Key
											</Label>
											<div className="flex gap-2">
												<Input
												...
												<Button
													...
													variant="outline"
													size="sm"
													onClick={handleValidateNewapi}
													disabled={
														isValidatingNewapi ||
														!newapiKey.trim() ||
														!newapiBaseUrl.trim() ||
														(!!validatedKeys.newapi &&
															validatedKeys.newapi === newapiKey.trim())
													}
												>
													{isValidatingNewapi ? (
														t("customKey.validating")
											...
											<p className="text-[11px] text-[var(--text-muted)]">
												配置您的 New API 实例地址和 API Key，支持通过 New API
												聚合的所有模型
											</p>
										</div>
									</section>
									...
									{availableModelPool.length > 0 && (
										<section className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-4 space-y-4">
											<div>
												<h3 className="text-sm font-medium text-[var(--text-primary)]">
													{t("customKey.modelConfig.title")}
												</h3>
												<p className="text-xs text-[var(--text-muted)] mt-1">
													{t("customKey.modelConfig.description")}
												</p>
											</div>

											<div className="grid gap-3 sm:grid-cols-2">
												<div className="space-y-1.5">
													<Label htmlFor="generator-model" className="text-xs">
														{t("customKey.modelConfig.generator")}
													</Label>
													<Select
														value={
															availableModelPool.some(
																(r) => r.model === generatorModel,
															)
																? generatorModel
																: ""
														}
													...
													>
														<SelectTrigger id="generator-model">
															<SelectValue
																placeholder={t("customKey.selectModel")}
															/>
														</SelectTrigger>
															...
															{availableModelPool.map((r) => (
																<SelectItem
																	...
																	description={
																		r.provider === "zenmux"
																			? "Zenmux"
																			: t("customKey.dashscope.short")
																	}
																	icon={getModelLogoPath(r)}
																/>
															))}
														</SelectContent>
													</Select>
												...
												<div className="space-y-1.5">
													<Label htmlFor="summary-model" className="text-xs">
														{t("customKey.modelConfig.summary")}
													</Label>
													<Select
														value={
															availableModelPool.some(
																(r) => r.model === summaryModel,
															)
																? summaryModel
																: ""
														}
												...
												<div className="space-y-1.5 sm:col-span-2">
													<Label htmlFor="review-model" className="text-xs">
														{t("customKey.modelConfig.review")}
													</Label>
													<Select
														value={
															availableModelPool.some(
																(r) => r.model === reviewModel,
															)
																? reviewModel
																: ""
														}
											...
											<div className="space-y-2">
												<Label className="text-xs">
													{t("customKey.modelConfig.candidates")}
												</Label>
												<p className="text-xs text-[var(--text-muted)]">
													{t("customKey.modelConfig.candidatesDesc")}
												</p>
												<DropdownMenu
												...
												>
													<DropdownMenuTrigger asChild>
														...
														>
															...
															<CaretDown
																size={16}
																className={`shrink-0 transition-transform ${isModelSelectorOpen ? "rotate-180" : ""}`}
															/>
														</button>
													</DropdownMenuTrigger>
													<DropdownMenuContent
															...
															<DropdownMenuCheckboxItem
																...
																onCheckedChange={(checked) =>
																	setSelectedModelsState((prev) =>
																		checked
																			? [...prev, r.model]
																			: prev.filter((m) => m !== r.model),
																	)
																}
															>
																<img
																	src={getModelLogoPath(r)}
																	alt=""
											...
											<div className="space-y-1.5">
												...
												<div className="flex gap-2">
													...
													<Button
														...
														aria-label={
															showMinimaxKey
																? t("customKey.minimax.hide")
																: t("customKey.minimax.show")
														}
													>
														{showMinimaxKey ? (
															<EyeSlash size={16} />
											...
											<div className="space-y-1.5">
												...
												<div className="flex gap-2">
													<Input
														...
														onChange={(e) =>
															setMinimaxGroupIdState(e.target.value)
														}
														className="flex-1"
													/>
													<Button
														...
														aria-label={
															showMinimaxGroupId
																? t("customKey.minimax.hideGroup")
																: t("customKey.minimax.showGroup")
														}
													>
														{showMinimaxGroupId ? (
															<EyeSlash size={16} />
